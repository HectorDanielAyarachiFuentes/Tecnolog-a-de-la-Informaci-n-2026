...
var Module={
    preRun:[],
    postRun:[function(){
        var el=document.getElementsByClassName("pleasewait")[0];
    ...
    canvas:function(){
        ...
        return e.addEventListener("webglcontextlost",(function(e){}),!1),e
    }(),
    setStatus:function(e){
        if(!e||e==="Running..."){
            var el=document.getElementsByClassName("pleasewait")[0];
            if(el)el.style.display="none";
            try { document.getElementById("canvas").focus(); } catch(err){}
        }
    },
    totalDependencies:0,
...
document.addEventListener("keydown", function(event)
{
    ...
    {
        const keyZ = new KeyboardEvent("keydown", { key: "z", code: "KeyZ", which: 90, keyCode: 90, shiftKey: false, ctrlKey: false, altKey: false, metaKey: false, bubbles: true, cancelable: true});
        document.getElementById("canvas").dispatchEvent(keyZ);
    }
    else if (k === "r")
    {
...
document.addEventListener("keyup", function(event)
{
    ...
    {
        const keyZ = new KeyboardEvent("keyup", { key: "z", code: "KeyZ", which: 90, keyCode: 90, shiftKey: false, ctrlKey: false, altKey: false, metaKey: false, bubbles: true, cancelable: true});
        document.getElementById("canvas").dispatchEvent(keyZ);
    }
    else if (k === "r")
    {
